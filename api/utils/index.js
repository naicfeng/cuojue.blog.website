...
const fn = {
  ...
  getIpRegion({ ip, detail = false }) {
    ...
    try {
      ...
      const [country, , province, city, isp] = region.replace(/0/g, '未知').split('|')
      // 有省显示省，没有省显示国家
      const area = province === '未知' ? country : province
      if (detail) {
        return [country, province, city, isp].join(' ').replace(/未知/g, '').trim()
      } else {
        return area === '未知' ? '' : area.replace(/(省|市)$/, '')
      }
    } catch (e) {

...
module.exports = async (request, response) => {
  ...
  console.log('请求 IP：', getRealIP(request))
    ...
    switch (event.event) {
      ...
      default:
        if (event.event) {
          ...
          res.message = 'Welcome to Cuojue\'s Comment'
          res.version = VERSION
        }
    }
  } catch (e) {
  ...
  try {
    ...
    await Promise.race([
      axios.post(`https://${process.env.VERCEL_URL}`, {
        event: 'POST_SUBMIT',
        comment
      }, { headers: { 'x-twikoo-recursion': config.ADMIN_PASS || 'true', 'x-twikoo-isadmin': isAdmin() } }),
...
async function postSubmit(comment, request) {
  ...
  // 垃圾检测
  if (!request.headers['x-twikoo-isadmin']) {
...
async function parse(comment, request) {
  ...
  const commentDo = {
    ...
    ua: comment.ua,
    ip: isAdminUser ? '0.0.0.0' : getRealIP(request),
